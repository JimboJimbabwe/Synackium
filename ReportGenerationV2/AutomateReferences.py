...
class ReportAutomation:
    ...
    def process_step(self, step_data: Dict):
        ...
        for idx, image in enumerate(step_data["ReferenceImages"], 1):
            print(f"\n3.{idx} Processing image: {image}")
            ...
            # Click filename box and enter filename
            print(f"  - Entering filename: {image}")
            self.click_coordinate(*self.coordinates["filename"], "filename box")
            time.sleep(3)
            self.type_text(image, "filename")
            time.sleep(2)
