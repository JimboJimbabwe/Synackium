...
class CombinedApp:
    ...
    def create_main_interface(self):
        # Main vertical paned window to separate top and bottom sections
        self.vertical_paned = ttk.PanedWindow(self.root, orient=tk.VERTICAL)
        self.vertical_paned.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        
        # Top section (original interface)
        self.top_paned = ttk.PanedWindow(self.vertical_paned, orient=tk.HORIZONTAL)
        ...
        self.top_paned.add(self.screenshot_frame, weight=1)
        self.top_paned.add(self.editor_frame, weight=2)
        
        # Bottom section (URL text display and controls)
        self.bottom_frame = self.create_url_panel()
        
        # Add both sections to vertical paned window
        self.vertical_paned.add(self.top_paned, weight=2)
        self.vertical_paned.add(self.bottom_frame, weight=1)
        
        # Menu
        self.create_menu()

    def create_screenshot_panel(self):
        frame = ttk.LabelFrame(self.top_paned, text="Screenshot Tool")
        ...
        self.viewfinder_window.geometry("1600x900")
    ...
    def create_mission_editor(self):
        frame = ttk.LabelFrame(self.top_paned, text="Mission Editor")
    ...
    def create_url_panel(self):
        frame = ttk.LabelFrame(self.vertical_paned, text="URL Selection")
        
        # Control buttons
        controls = ttk.Frame(frame)
        controls.pack(fill=tk.X, padx=5, pady=5)
        
        ttk.Button(controls, text="Load URLs", 
                  command=self.load_urls).pack(side=tk.LEFT, padx=5)
        ttk.Button(controls, text="Save Selected", 
                  command=self.save_selected_urls).pack(side=tk.LEFT, padx=5)
        
        # Text widget for URL display
        self.url_text = Text(frame, wrap=tk.NONE, height=10)
        self.url_text.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        
        # Add scrollbars
        y_scrollbar = ttk.Scrollbar(frame, orient=tk.VERTICAL, command=self.url_text.yview)
        y_scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        x_scrollbar = ttk.Scrollbar(frame, orient=tk.HORIZONTAL, command=self.url_text.xview)
        x_scrollbar.pack(side=tk.BOTTOM, fill=tk.X)
        
        self.url_text.configure(yscrollcommand=y_scrollbar.set, xscrollcommand=x_scrollbar.set)
    ...
    def show_image_preview(self, event):
        ...
        if index:
            ref_path = self.normalize_path(self.references[index[0]])
            try:
    ...
    def set_save_path(self):
        ...
        if path:
            self.save_path = self.normalize_path(path)
    ...
    def take_screenshot(self):
        ...
        try:
            ...
            filepath = self.normalize_path(os.path.join(self.save_path, filename))
            im.save(filepath)
    ...
    def load_existing_references(self):
        self.references = [
            self.normalize_path(os.path.join(self.save_path, f))
    ...
    def save_mission(self):
        mission_data = []
        for step in self.steps:
            # Normalize all reference paths before saving
            normalized_refs = [self.normalize_path(ref) for ref in step['references']]
            mission_data.append({
                "Step": int(step['number']),
                "Action": step['action'],
                "ReferenceImages": normalized_refs,
    ...
    def load_urls(self):
        ...
        try:
            ...
            messagebox.showerror("Error", "raw.txt file not found!")
        except Exception as e:
            messagebox.showerror("Error", f"Error loading URLs: {str(e)}")
            
    def save_selected_urls(self):
        """Save selected URLs to assets.txt file"""
        try:
            # Get selected text
            ...
            messagebox.showinfo("Success", "Selected URLs saved to assets.txt")
        except tk.TclError:  # No selection
            messagebox.showwarning("Warning", "No text selected!")
        except Exception as e:
            messagebox.showerror("Error", f"Error saving URLs: {str(e)}")
